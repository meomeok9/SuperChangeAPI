...
module.exports.login = async(req,res,next)=>{
    ...
    try{
        ...
        if(!exitsUser) return res.status(401).json({message:"USER NOT FOUND",isLoggIn : false});
        else{
           bcrypt.compare(password, exitsUser.password)
           .then((isMatch)=>{
               if(isMatch){
                   return res.status(200).json({message : "SUCCESS to LOGIN", isLoggIn:true});
               }else return res.status(401).json({message:"PASSWORD NOT CORRECT",isLoggIn : false});
           })
           .catch(err=> res.status(403).json({message:"Some thing wrong", isLoggIn : false}));
        }
